...
SELECT DENSE_RANK() OVER(ORDER BY cnt DESC) AS rank_message, firstname, lastname, cnt FROM (
SELECT u.firstname, u.lastname, COUNT(
CASE 
WHEN m.from_user_id = u.id THEN 0
ELSE null
END) AS cnt FROM users u
...
SELECT DENSE_RANK() OVER(ORDER BY cnt DESC) AS rank_message, firstname, lastname, cnt FROM (
SELECT u.firstname, u.lastname, COUNT(
IF(m.from_user_id = u.id, 0, NULL)) AS cnt FROM users u
LEFT JOIN messages m ON m.from_user_id = u.id
GROUP BY u.id
ORDER BY cnt DESC
) AS rank_message1
...
SELECT m.id, m.created_at, LEAD(m.created_at) OVER (ORDER BY m.created_at) AS next, 
	timestampdiff(MINUTE, m.created_at, LEAD(m.created_at) OVER (ORDER BY m.created_at)) AS diff
FROM messages m
