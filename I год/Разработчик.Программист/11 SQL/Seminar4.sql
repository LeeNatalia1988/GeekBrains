...
SELECT IF(profiles.gender = "m", "мужской", "женский") AS gender, COUNT(*) FROM profiles
...
Найдите количество друзей у каждого пользователя. Выведите для каждого пользователя идентификатор пользователя, фамилию, имя и количество друзей. Сортировка 
выводимых записей в порядке возрастания идентификатора пользователя.
*/


SELECT users.id, users.firstname, users.lastname, COUNT(
CASE 
WHEN friend_requests.status = "approved" THEN 0
ELSE NULL
END) AS cnt FROM users
LEFT JOIN friend_requests ON friend_requests.initiator_user_id = users.id OR friend_requests.target_user_id = users.id
GROUP BY(users.id)
ORDER BY(users.id);
