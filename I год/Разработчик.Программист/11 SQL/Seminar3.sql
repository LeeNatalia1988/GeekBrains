...
SELECT post, SUM(salary) AS "Общая сумма зарплаты" FROM staff
GROUP BY post;

SELECT COUNT(*) AS "количество" FROM staff 
WHERE post = "Рабочий" AND 24 <= age AND age <= 49;  

SELECT COUNT(DISTINCT post) AS "Количество уникальных специальностей" FROM staff;
